...
def calculate_average_rating(reviews: List[ReviewGet]) -> float:
...
class SongService:
    ...
    def get_song(song_id: str):
        song = db.find_by_id("songs", song_id)
    ...
    def get_top_rated_songs(limit: int):
        ...
        for song in grouped_reviews:
            grouped_reviews[song]["avg_rating"] = (
                grouped_reviews[song]["avg_rating"] /
                grouped_reviews[song]["count"]
            )

        # top_rated_songs = sorted(grouped_reviews, key=lambda x: x['avg_rating'], reverse=True)[:limit]
        top_rated_songs = sorted(
            grouped_reviews.items(), key=lambda x: x[1]["avg_rating"], reverse=True
        )[:limit]
        # top_song_names = [song[0] for song in top_rated_songs]
        result = [
            {"song": song[0], "average_rating": song[1]["avg_rating"]}
